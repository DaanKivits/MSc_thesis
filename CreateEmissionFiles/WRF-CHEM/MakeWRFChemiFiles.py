...
# This script creates WRFchemi flux files based on the grid definitions that results from running the WPS geogrid algorithm,
...
# The script therefore does not need any existing WRFChemi emission files as input. In this example 'geo_em.d01.nc' is used to 
# extract the WPS grid information, but the 'met_em.d01.nc' or other WPS output file could also be used for the same purpose.
# Some points of attention:
#   - WPS should have ran at least once before we are able to create the WRFchemi emission files from scratch with this script,
#       which can be done by either running CTDAS-WRF with the WPS namelist files that contains the right domain configuration,
#       or by running geogrid.exe separately before running this script. If this is not done, the script won't be able to find
#       the 'geo_em.d01.nc' file!
...
# To circumvent any overwriting errors, remove the target directory and create a new empty one 
...
ref_lon = (geo.variables['XLONG_U'][0,sn/2,we/2] + geo.variables['XLONG_U'][0,sn/2-1,we/2])/2.
ref_lat = (geo.variables['XLAT_V'][0,sn/2,we/2] + geo.variables['XLAT_V'][0,sn/2,we/2-1])/2.
truelat1 = geo.TRUELAT1
truelat2 = geo.TRUELAT2
dx_dom = geo.DX
domain = 1

# Define Lambert Conformal Concic projection basemap to prepare for transformation later
m = Basemap(width=dx_dom*we,height=dx_dom*sn,
            rsphere=(6378137.00,6356752.3142),\
            resolution='l',area_thresh=1000.,projection='lcc',\
            lat_1=truelat1,lat_2=truelat2,lat_0=ref_lat,lon_0=ref_lon)

## Define Equidistant lon-lat projection basemap to prepare for transformation later
...
nx = 344 # CTE-HR gridsize in x dimension (amount of gridcells)
ny = 435 # CTE-HR gridsize in y dimension (amount of gridcells)
