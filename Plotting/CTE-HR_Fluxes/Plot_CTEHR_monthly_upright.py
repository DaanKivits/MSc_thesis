...
# A simple plotting script used to vertically (3 rows, 2 columns) plot the average monthly CTE-HR flux anomalies for the growing season of different 
...
plt.subplots_adjust(left = 0.03, top = 0.95, bottom = 0.04, right=0.97, hspace = 0.12, wspace = 0.01)
axs = ax.ravel()
...
#for i in range(0, len(months)):
    ...
    axs[i].set_title(calendar.month_name[months[i]], size=10, weight = 'bold')
    ...
    if i == len(fluxstring)-1:
        cbaxes = fig.add_axes([0.03, 0, 0.94, 0.03]) #Add position (left, bottom, width, height)
...
plt.savefig("/projects/0/ctdas/dkivits/figures/drought/Drought_anomaly_multimonth_" + fluxfile_basename + ".png", dpi=300, bbox_inches='tight')
plt.savefig("/projects/0/ctdas/dkivits/figures/drought/Drought_anomaly_multimonth_" + fluxfile_basename + ".pdf", dpi=300, bbox_inches='tight')
plt.show()
